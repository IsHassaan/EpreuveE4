<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../css/experiences.module.css?v=<?php echo time() ?>">
    <title>Experiences</title>
</head>
<?php include 'navbar.php'; ?>
<body>

    <div class="introduction_experiences">
        <h2>Situation professionnelle</h2>
        <p>
        Dans cette partie, nous verrons le contexte et les enjeux liés 
        au stage effectué durant les semaines qui ont 
        composées<br>
        la partie pratique de la formation en BTS SIO<br>
        Il s'agit notamment de comprendre les besoins, les situations et les compétences 
        qui ont été abordées lors du passage en entreprise.
        </p>
    </div>


    <div class="stage">
        <div class="entete">
            <div class="entete_text">
            <h3>Stage 2022-2023</h3>
                        <h3>Keozia</h3>
                        <h4>Informations sur l'entreprise : </h4>
                        <p>
                        Keozia est une entreprise d'édition de logiciels applicatifs fondée en mai 2022
                        par Michael Sanquer située à Londre.<br>
                        L'entreprise compte une multitude de sites web basés sur des concepts et systèmes différents les uns des autres<br>
                        Le site web sur lequel le stage a été effectué est basé sur l'apprentissage des compétences liées 
                        au développement informatique et à la gestion d'une plateforme web.
                        Le site étant assez récent, le stage s'est principalement concentré sur la création de nouvelles fonctionnalités.
                        </p>
                        <h4>Rôle dans l'entreprise :</h4>
                        <p>
                        Développeur PHP en charge de la modification et de la création de module et de fonctions
                        pour le site webmaster.com.<br>
                        Création d'interface et mise en place de nouvelles pages, d'API
                        et autres éléments tiers permettant notamment l'accueil de moyens de paiements et de
                        systèmes d'envoi de factures automatisé.<br>
                        Gestion des évènements utilisateurs et modifications de données en conséquence.
                        </p>
            </div>
            <img src="../assets/semwee.jpg" alt="logo">
        </div>
        <h4>Compétences concernées : </h4>
        <div class="competences">
            <div class="competence">
                <img src="../assets/php.png" alt="logo">
                <p>
                Connaissance de la syntaxe PHP.
                Création de fonctions et méthodes liées aux interactions 
                utilisateurs.
                Communication avec les API et la base de données.
                </p>
            </div>
            <div class="competence">
            <img src="../assets/htmlcss.png" alt="logo">
                <p>
                Mise en place et modification des éléments de l'interface graphique
                de l'application web.
                </p>
            </div>
            <div class="competence">
            <img src="../assets/mysql.png" alt="logo">
                <p>
                Gestion des données de l'application développée.
                Modification, ajout et suppression des données de l'application 
                et liaison des fonctions à la base de données.
                </p>
            </div>
            <div class="competence">
            <img src="../assets/api.png" alt="logo">
                <p>
                Gestion des bibliothèques externes et importation de fonctionnalités tiers.
                Liaison des fonctionnalités à la base de données du projet.
                </p>
            </div>
        </div>

        <h4>Méthode de l'entreprise : </h4>
        <div class="methode">
            <p>
            Les méthodes de l'entreprise se basent sur un suivi quotidien des tâches attribuées au
            stagiaire à travers des réunions de présentation et de démonstration du travail effectué la veille.<br>
            Chaque fonction testée puis ajouter au serveur via FileZilla afin d'implémenter celle-ci
            au projet final après validation du tuteur.<br>
            Le stagiaire possède plusieurs tâches à effectuer sans ordre précis, la priorité est attribuée
            à la finalisation de la tâche dans le délai accordé. 
            </p>
        </div>
        <a href="https://www.linkedin.com/company/semwee/" class="lien_entreprise">LinkedIn</a>
    </div>


    <div class="stage">
        <div class="entete">
            <div class="entete_text">
            <h3>Stage 2021-2022</h3>
                        <h3>Semwee</h3>
                        <h4>Informations sur l'entreprise : </h4>
                        <p>
                        Semwee est une entreprise d'édition de logiciels applicatifs fondée en 2021
                        par Anthony Contat située à Paris.<br>
                        Le but de l'entreprise est de créer une application web qui répond à des besoins en termes
                        de SEO.<br>
                        L'application sur laquelle le stage a été effectué est basée sur le langage Angular et permet 
                        une analyse des sites clients afin de leur proposer les changements nécessaires à l'adoption 
                        d'un meilleur référencement.
                        </p>
                        <h4>Rôle dans l'entreprise :</h4>
                        <p>
                        Développeur Angular en charge de la création et de la modification 
                        de fonctions et méthodes en lien avec les interactions clients enregistrées 
                        sur une base de données MongoDB.
                        Correction de bug et ajout de fonctionnalités qui comportent une intervention sur 
                        le front-end ainsi que le back-end de la solution logiciel.
                        </p>
            </div>
            <img src="../assets/semwee.jpg" alt="logo">
        </div>
        <h4>Compétences concernées : </h4>
        <div class="competences">
            <div class="competence">
                <img src="../assets/angular.png" alt="logo">
                <p>
                Connaissance de la syntaxe et des règles autour d'Angular
                et de la communication avec les modules qui accompagnent le projet.
                </p>
            </div>
            <div class="competence">
            <img src="../assets/htmlcss.png" alt="logo">
                <p>
                Mise en place et modification des éléments de l'interface graphique
                de l'application web.
                </p>
            </div>
            <div class="competence">
            <img src="../assets/mongodb.png" alt="logo">
                <p>
                Gestion des données de l'application développée.
                Modification, ajout et suppression des données de l'application 
                et liaison des fonctions à la base de données. 
                </p>
            </div>
            <div class="competence">
            <img src="../assets/javascript.png" alt="logo">
                <p>
                Création de fonctions en lien avec la base de données dans
                le but de gérer les évènements lors des interactions de l'utilisateur.
                </p>
            </div>
        </div>

        <h4>Méthode de l'entreprise : </h4>
        <div class="methode">
            <p>
            Les méthodes de l'entreprise se basent sur la méthode agile dite, SCRUM.
            Chaque développeur est affecté à une tâche en fonction d'un ordre d'importance 
            et de difficulté.<br>
            Tâche dont il informe son équipe de l'avancée de manière quotidienne à travers 
            des réunions d'entreprise qui permettent de faire le point sur l'état du projet
            par rapport à la progression des individualités.<br>
            Grâce à cette méthode, chacun se voit assigner une mission propre à ses compétences ainsi
            qu'à la durée de son contrat.
            </p>
            <p>
            Afin de suivre l'avancer de chacun, les modifications apportées au projet sont 
            ajoutées à la branche du concerné sur le GitHub de l'application, ensuite les informations 
            sur celle-ci sont renseignées sur un Trello en fin de journée dans le but de laisser le plus
            d'information aux autres développeurs en cas de reprise de la tâche par un développeur 
            plus compétent ou ayant plus de temps pour remplir la tâche attribuée.
            </p>
        </div>
        <a href="https://fr.linkedin.com/in/michael-sanquer-0b493b259?trk=public_profile_samename-profile&original_referer=https%3A%2F%2Fwww.google.com%2F" class="lien_entreprise">LinkedIn</a>
    </div>




<?php include 'contact.php'; ?>
</body>
</html>